...
import { mergeConfig, enableStreamingMode } from './utils/config-utils';
...
export default class HlsBase implements HlsEventEmitter {
  ...
  protected coreComponents: ComponentAPI[];
  ...
  /**
   * Get or set the default configuration used by hls.js, which is extended/overriden
   * by `userConfig` when instantiating a `new Hls(userConfig)` instance.
   * NOTE: automatically set to the standard Hls default config in the `Hls` sublcass.
   */
  static get DefaultConfig(): HlsConfig {
    if (!HlsBase.defaultConfig) {
      throw new TypeError('HlsBase.DefaultConfig must be defined.');
    }
    return HlsBase.defaultConfig;
  }
